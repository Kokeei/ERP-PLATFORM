...
- `logement/index.html` : module Gestion Locative, regroupant les cartes portail « Locataires » et « Demandeurs » — un dossier de demande attribué devient automatiquement un locataire avec un bail, donc une application unique plutôt que deux silos. Référentiels `logements` (patrimoine), `demandeurs` (workflow Déposée → Instruction → Éligible → Proposée → Attribuée, avec proposition d'un logement vacant puis attribution qui crée le locataire et le bail), `locataires`, `baux`, `quittances` (génération mensuelle sans doublon, comme la paie du module RH) et `interventions` (demandes de travaux). Profils simulés : Gestion locative (accès complet), Locataire (self-service : bail, quittances, signaler un problème) et Demandeur (suivi de son dossier). Les cartes « Locataires » et « Demandeurs » du portail pointent vers `logement/#locataires` / `#demandeurs`.
