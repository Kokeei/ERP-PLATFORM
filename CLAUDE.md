...
- Persistance : `localStorage` du navigateur uniquement (pas de backend). Les données de démonstration sont régénérées via le bouton « Reset ».
- Déploiement : GitHub Pages, servi automatiquement depuis `main` (aucune étape de build).
- Tests E2E : Playwright (`tests/`, `npm test`) — voir `tests/README.md`. `tests/agent-rh/` couvre le portail self-service agent (navigation, demandes RH, tâches, notifications, tableau de bord).
